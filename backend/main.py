...
async def auth_callback(request: Request, db: Session = Depends(get_db)):
    ...
    if 'error' in params:
        return RedirectResponse(f"http://localhost:8080/login.html?error={params.get('error')}")
    
    code = params.get('code')
    if not code:
        return RedirectResponse("http://localhost:8080/login.html?error=missing_code")
    ...
    db.commit()
    return RedirectResponse(f"http://localhost:8080/dashboard.html?success=true&user_id={user.id}")
...
async def get_users(db: Session = Depends(get_db)):
    ...
    return {"success": True, "users": [{"id": u.id, "email": u.email, "name": u.name} for u in users]}
...
async def sync_emails(user_id: int, max_results: int = 50, db: Session = Depends(get_db)):
    # 檢查用戶是否存在
    user = db.query(User).filter(User.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
...
async def get_user_emails(user_id: int, limit: int = 10, db: Session = Depends(get_db)):
    emails = db.query(Email).filter(Email.user_id == user_id).order_by(Email.received_at.desc()).limit(limit).all()
...
async def gmail_status(user_id: int, db: Session = Depends(get_db)):
    ...
    interview_count = db.query(Email).filter(Email.user_id == user_id, Email.is_interview_related == True).count()
...
async def analyze_email(email_id: int, db: Session = Depends(get_db)):
    ...
    is_interview, confidence = openai_service.is_interview_email(email.subject or "", email.body_text or "")
...
async def extract_interview_info(email_id: int, db: Session = Depends(get_db)):
    ...
    openai_service = get_openai_service()
    interview_info = openai_service.extract_interview_info(email.subject or "", email.body_text or "")
    
    if not interview_info:
        return JSONResponse({"success": False, "error": "extraction_failed"}, status_code=400)
    
    existing_invitation = db.query(InterviewInvitation).filter(InterviewInvitation.email_id == email_id).first()
...
async def generate_reply(email_id: int, tone: str = "professional", db: Session = Depends(get_db)):
    ...
    # 智能檢查：如果還沒有面試資訊，自動提取
    invitation = db.query(InterviewInvitation).filter(InterviewInvitation.email_id == email_id).first()
    ...
    return {
        ...
        "body": reply_body,
        "tone": tone,
        "auto_extracted": not invitation or invitation.created_at == invitation.created_at
    }

if __name__ == "__main__":
    print("Starting Interview Assistant API...")
    print("前端: http://localhost:8080/login.html")
